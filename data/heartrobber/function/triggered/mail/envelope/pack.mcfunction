...
$item modify entity @s weapon.offhand {function:set_components,components:{max_stack_size:1,item_model:"heartrobber:envelope",custom_data:{"heartrobber:type":19b,"heartrobber:sent":false,"heartrobber:envelope":{id:"$(id)",components:$(components),count:$(count)}},item_name:{"translate":"item.heartrobber.envelope"},"!custom_name":{}}}
