...
execute if score @s heartrobber_compare matches 3 if block ~1 ~ ~ dispenser[facing=west] positioned ~-1 ~ ~ if predicate heartrobber:cocoa_placeable/x run return 3
execute if score @s heartrobber_compare matches 4 if block ~ ~ ~1 dispenser[facing=north] positioned ~ ~ ~-1 if predicate heartrobber:cocoa_placeable/z run return 4
return 0